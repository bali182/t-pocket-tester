...
const createInput = ({
  ...
  rootRadius = 0,
  rootWidth = 12,
  usePocketCluster = false,
}: Options): MagicFixIssueDetectorInput => {
  const subProject = createSubProject({ closed, rootRadius, rootWidth, usePocketCluster })
  const project = createProject(subProject)
  const config = withMinimumDistance(
    createMagicFixConfig(project, subProject),
    usePocketCluster ? clusterId : rootId,
    minimumDistance,
    accuracy,
  )
...
const withMinimumDistance = (
  ...
  componentConfigs: Object.fromEntries(
    Object.entries(config.componentConfigs).map(([id, value]) => [
      id,
      { ...value, preferredMinimumDistanceFromEdge: id === componentId ? minimumDistance : 0 },
    ]),
  ),
})

const createProject = (subProject: SubProjectSchema): ProjectSchema => ({
  id: 'project',
  name: 'Project',
  ...
  componentSettings: { baseColor: '#000' },
  stitchingSettings: {
    stitchMargin: 1,
    stitchHoleLength: 1,
...
const createSubProject = ({
  ...
  rootWidth,
  usePocketCluster,
}: Required<Omit<Options, 'accuracy' | 'minimumDistance'>>): SubProjectSchema => {
  ...
  const root: RootPanelSchema = {
    ...
    id: rootId,
    name: 'Root',
    width: usePocketCluster ? 100 : rootWidth,
    height: 100,
    layoutOrientation: 'horizontal',
    layoutOrder: 'default',
    layoutGap: 0,
    children: usePocketCluster ? [clusterId] : [],
    borderRadius: rootRadius,
    topLeftRadius: rootRadius,
    topRightRadius: rootRadius,
    bottomRightRadius: rootRadius,
    bottomLeftRadius: rootRadius,
    individualRadii: false,
  }
  const componentLine: ComponentBoundsStitchLineSchema = {
    ...
    bottom: closed,
    left: closed,
    topLeftCorner: closed,
    topRightCorner: closed,
    bottomRightCorner: closed,
    bottomLeftCorner: closed,
    topStartOffset: 0,
    topEndOffset: 0,
    rightStartOffset: 0,
    rightEndOffset: 0,
  ...
  const pocketLine: PocketClusterStitchLineSchema = {
  ...
  return {
    id: 'sub',
    root: rootId,
    components: usePocketCluster ? { [rootId]: root, [clusterId]: cluster } : { [rootId]: root },
    holes: [],
    stitchLines: usePocketCluster ? [pocketLine] : [componentLine],
  }
}
