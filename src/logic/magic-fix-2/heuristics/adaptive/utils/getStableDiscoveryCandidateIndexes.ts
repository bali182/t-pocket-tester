...
/** FNV-1a's 32-bit offset basis; turns a field path into a stable initial state. */
const FNV_1A_32_OFFSET_BASIS = 2_166_136_261
/** FNV-1a's 32-bit prime; mixes each path character into the hash. */
...
export const getStableDiscoveryCandidateIndexes = (
  path: AdaptiveMagicFixFieldPath,
  candidateCount: number,
  discoveryIterationCount: number,
): number[] => {
  const indexes = Array.from({ length: discoveryIterationCount }, (_, index) => index % candidateCount)
  const nextUint32 = createNextUint32(getStringHash(JSON.stringify(path)))
